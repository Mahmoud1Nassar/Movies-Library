...
app.post("/addMovie", async (req, res) => {
  ...
  client
    ...
    .catch((error) => {
      ...
      res
        .status(500)
        .json({ message: "Unable to add movie", error: error.toString() });
    });
});
...
app.get("/getMovies", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Unable to fetch movies from the database",
...
app.put("/update/:id", async (req, res) => {
  ...
  client
    ...
    .catch((error) => {
      ...
      res
        .status(500)
        .json({ message: "Unable to update movie", error: error.toString() });
    });
});
...
app.delete("/deleteMovies/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Unable to Delete movies from the database",
        error: error.toString(),
      });
  }
});
...
app.get("/getMovie/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Unable to fetch movie from the database" });
  }
});
...
app.get("/search", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Unable to search movies", error: error.toString() });
  }
});
...
app.get("/top-rated-movies", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Unable to retrieve top-rated movies",
        error: error.toString(),
      });
  }
});
...
app.get("/", (req, res) => {
  const responseMovie = new Movie(
    "Spider-Man: No Way Home",
    `/1g0dhYtq4irTY1GPXvft6k4YLjm.jpg`,
    "Peter Parker is unmasked and no longer able to separate his normal life from the high-stakes of being a superhero. When he asks for help from Doctor Strange, the stakes become even more dangerous, forcing him to discover what it truly means to be Spider-Man."
  );
...
app.use((err, req, res, next) => {
  res
    .status(500)
    .json({ status: 500, responseText: "Sorry, something went wrong" });
});
